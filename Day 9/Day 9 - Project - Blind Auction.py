...
if clear_console: os.system("cls")
...
def valid_name_checker(name):
    ...
    if not name.isalpha():
        invalid_name     = True
        invalid_name_msg = ("The name you entered contained a symbol not in the English alphabet."
            + "\n"
    ...
    while invalid_name:
        ...
        if name.isalpha(): invalid_name = False
        else: print(invalid_name_msg)
...
def valid_bid_checker(bid):
    ...
    if invalid_bid:
        print("The bid you entered was invalid."
        + "\n"
...
sorted_bidder_tuples = sorted(bidder_tuples, reverse=True, key=lambda bidder_tuples: bidder_tuples[1])
...
# and a winner of the tie is chosen randomly.
    ...
    print("There was a tie for highest bid! Randomly choosing a winner among the highest bidders.")
