...
def coffee_machine():
    ...
    def request_money(cost_of_drink: int):
        ...
        while True:
            ...
            if money >= cost_of_drink:
                ...
                print(
                    f"You inserted ${money:.2f}. You need ${diff:.2f} more to buy this drink."
                )
                num_quarters, num_dimes, num_nickles, num_pennies = (
                    0,
                    0,
                    0,
                    0,
                )  # Reset num of each coin.
    ...
    def has_res_for_drink(watr_need, coff_need, milk_need, no_msg=False):
        ...
        if c_watr < watr_need:
            if not no_msg:
                print("Sorry machine doesn't have enough water for that drink.")
            not_enough_resources = True
        if c_coff < coff_need:
            if not no_msg:
                print("Sorry machine doesn't have enough coffee for that drink.")
            not_enough_resources = True
        if c_milk < milk_need:
            if not no_msg:
                print("Sorry machine doesn't have enough milk for that drink.")
            not_enough_resources = True
    ...
    while running:
        ...
        drink_cost, water_needed, coffee_needed, milk_needed, user_choice = (
            get_user_choice()
        )
        ...
        if user_choice in MENU:
            print(
                f"You selected {user_choice.capitalize()}, this drink cost: ${drink_cost:.2f}"
            )
        ...
        if (
            has_res_for_drink(watr_need=50, coff_need=18, milk_need=0, no_msg=True)
            != True
        ):
