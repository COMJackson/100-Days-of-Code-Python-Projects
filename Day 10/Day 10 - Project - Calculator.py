...
def ask_for_ending_action(num):
    ending_msg = (f"To continue using your current result ({num}) in the calculator, type \"continue\" or \"c\".\n"
...
def calculator():
    ...
    while calculator_running:
        ...
        # ask the user which side of the operator the current answer should be on.
            ...
            if operation_sym in ("/", "-"):
                print(f"Please choose the side of the operator to put the current result ({stored_result}) on.")
                stored_result_side = input("Type \"l\" for left or \"r\" for right: ").casefold()
            else:
