...
from time import (
    sleep,
)  # User when making random choices to delay the system clock by a random float between 0 and 1.
from random import (
    choice,
    uniform,
)  # Used to get a random choice from a list and to get a random float.
from copy import (
    deepcopy,
)  # Used to create deep copies to not modify orignal of a iterable.
...
def get_data(local_data, starter_choice_a=None):
    ...
    if starter_choice_a != None:
        ...
        if (
            selected_data == starter_choice_a
        ):  # If the selected_data is the starter_choice_a recall the function.
...
def get_player_choices(
    local_data, current_correct_choice, current_choice_a, current_choice_b
):
    """
...
def did_player_win(passed_cho_a_followers, passed_cho_b_followers):
    ...
    players_choice = (
        get_player_input()
    )  # Gets the players choice and returns a str: "a" or "b".
...
def hi_lo_game():
    ...
    while alive:
        ...
        if player_winning:
            ...
            choice_a, choice_b = get_player_choices(
                local_data, correct_choice, choice_a, choice_b
            )
            ...
            cho_a_msg = (
                f"Choice 'A': {cho_a_name}, a {cho_a_desc}, from {cho_a_country}."
            )
            ...
            cho_b_msg = (
                f"Choice 'B': {cho_b_name}, a {cho_b_desc}, from {cho_b_country}.\n"
            )
            ...
            player_winning, correct_choice = did_player_win(
                cho_a_followers, cho_b_followers
            )
            ...
            if local_data == []:
                ...
                print(logo)  # Print the starting logo.
                print(
                    "We've run out of choices for you! I guess that means you win :)"
                )
